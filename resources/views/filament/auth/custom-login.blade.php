    ...
    <div class="max-w-md w-full space-y-8">
        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('filament.admin.auth.login') }}">
            ...
            <div class="rounded-md shadow-sm -space-y-px">
                <div>
                    <label for="email" class="sr-only">{{ __('filament-panels::pages/auth/login.form.email.label') }}</label>
                    <input id="email" name="email" type="email" autocomplete="email" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="{{ __('filament-panels::pages/auth/login.form.email.label') }}" value="{{ old('email') }}" />
                </div>
                <div>
                    <label for="password" class="sr-only">{{ __('filament-panels::pages/auth/login.form.password.label') }}</label>
            ...
            <div class="flex items-center justify-between">
                ...
                @if(filament()->hasPasswordReset())
                    <div class="text-sm">
                        <a href="{{ route('filament.admin.auth.password-reset.request') }}" class="font-medium text-indigo-600 hover:text-indigo-500">
            ...
            @if(filament()->hasRegistration())
                <div class="text-center">
                    <a href="{{ route('filament.admin.auth.register') }}" class="font-medium text-indigo-600 hover:text-indigo-500">
